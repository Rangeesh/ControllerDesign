...
totaltime = 62; %Time to run the simulation for.
...
f3 = fit(center(:,1),center(:,2),'smoothingspline');
...
if reversetrack
    [t_car,q_car] = ode45(@(t,q) bicycleModel(t,q,car,vx), 0:step:totaltime, [points(1,1),points(1,2),pi/2+pi/4-.3,0,0], options);
else
...
if drawrealtime
    for i = 1:length(t_car)
        ...
        hold on
        if i>1
            plot([q_car(i-1,1), q_car(i,1)],[steeranglenorm(i-1), steeranglenorm(i)],'b')
        end
